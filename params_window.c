...
void ParamsWindowInit()
{
    ...
    vec2 size = { 200, 400};
    vec2 position = {0, 100};
    WindowWidgetInit(&params_window, "Параметры", size, &dParam, position);
    params_window.resizeble = false;

    EWidgetText *label_texture_path = calloc(1, sizeof(EWidgetText));
    TextWidgetInitDefault(label_texture_path, 9, &dParam, &params_window);
    ...
    EntryWidgetInit(&entry_texture_path, 4, &dParam, &params_window);
    ...
    TextWidgetInitDefault(label_shader_frag_path, 9, &dParam, &params_window);
    ...
    EntryWidgetInit(&entry_shader_frag_path, 4, &dParam, &params_window);
    ...
    TextWidgetInitDefault(label_shader_vert_path, 9, &dParam, &params_window);
    ...
    EntryWidgetInit(&entry_shader_vert_path, 4, &dParam, &params_window);
    Transform2DSetPosition(&entry_shader_vert_path, 0, 260);
    Transform2DSetScale(&entry_shader_vert_path, 200, 30);

    size.y = 30;
    ComboboxWidgetInit(&topology_comb, size, &dParam, &params_window);
    Transform2DSetPosition(&topology_comb, 0, 360);
    Transform2DSetScale(&topology_comb, 200, 30);

    ComboboxWidgetAddItem(&topology_comb, "TOPOLOGY_LINE", &dParam);
    ComboboxWidgetAddItem(&topology_comb, "TOPOLOGY_TRIANGLE", &dParam);

    EWidgetButton *accept = calloc(1, sizeof(EWidgetButton));
    ButtonWidgetInit(accept, "Применить", &dParam, &params_window);
